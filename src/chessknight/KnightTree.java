...
/**
 * Class represents all possible moves of chess knights on the chess table
 * and methods for find the shortest path between two knights
 * 
...
public class KnightTree {
	static class KnightNode {
		...
		public String toString() {
			...
			if(availableMoves.size() > 0) {
				tmp+=" - Available moves ="+ availableMoves.size();
			}
			for(Iterator<KnightNode> it= availableMoves.iterator();it.hasNext();){
				KnightNode node = it.next();
				tmp+="";
				tmp+=node;
			}			
			return tmp;
	...
	public void printInfo() {
		...
		int i=0;
		for(Iterator<KnightNode> it= nodes.iterator() ;it.hasNext();i++){
			KnightNode node = it.next();
			info+=node;
			info+="\n--"+i+"--\n";
		}
	...
	public void printPath(KnightTree.KnightNode node, String prefix){
		
		KnightTree.KnightNode tmp = node;
	 ...
	 * @return
	 */
	public KnightTree.KnightNode getPathByDFS(KnightNode startNode, KnightNode finishNode){
		if( startNode == null) {
		...
		while(stack.size() > 0) {
			...
			for(Iterator<KnightNode> it = childNodes.iterator(); it.hasNext();) {
				KnightNode tmpChild = it.next();
	 ...
	 * @return
	 */
	public KnightTree.KnightNode getPathByBFS(KnightNode startNode, KnightNode finishNode){
		if( startNode == null) {
			throw new IllegalArgumentException("start node shouldn't be null!");
		}
	...
	public KnightTree.KnightNode findNode(String x, String y) {
		for(Iterator<KnightTree.KnightNode> it = nodes.iterator(); it.hasNext();) {
			KnightTree.KnightNode tmpNode = it.next();
	...
	public void testPositive()
	{
		...
		//print result path
			tree.printPath(resultNode, "BFS between:\n"+finish+"\n"+start);
		tree.resetTree();
